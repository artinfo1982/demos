...
if __name__ == '__main__' and sys.argv[1] == 'train':
    ...
    for i in range(0, NUM_CLASSES):
        # 这里可以改成你自己的图片目录，i为分类标签
        dir = './train_images/validation-set/chinese-characters/%s/' % i
        for rt, dirs, files in os.walk(dir):
            for filename in files:
                val_count += 1
...
if __name__ == '__main__' and sys.argv[1] == 'predict':
    ...
    with tf.Session() as sess:
        ...
        for n in range(1, 2):
            ...
            print ("概率：  [%s %0.2f%%]    [%s %0.2f%%]    [%s %0.2f%%]" % (PROVINCES[max1_index],
                                                                          max1 * 100, PROVINCES[max2_index], max2 * 100, PROVINCES[max3_index], max3 * 100))
